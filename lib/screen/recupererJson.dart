  ...
  try {
    ...
    for (int i = 0; i < hotelRepertorie.length; i++) {
      ...
      double distancesInMeters = Geolocator.distanceBetween(
          maLatitude,
          maLongitude,
          double.parse(hotelRepertorie[i].latitude),
          double.parse(hotelRepertorie[i].longitude));
...
/* class Restaurants {
  ...
  Restaurants(
      {this.id_part,this.nom_part, this.ville_part,this.longitude, this.latitude, this.heureO, this.heureF, this.telephone,this.infousersuper,
      this.article});
...
Future<List<Restaurants>> restaurantcopy(
    List<Restaurants> restaurantRepertorie) async {
  //var lisons = maListe.length;
  //print("${lisons}");
  // log('usecopieDebut: $usecopy');
  Location location;
