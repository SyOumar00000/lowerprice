...
class _DetailSupermarchePageState extends State<DetailSupermarche> {
      ...
      body: Column(
        children: [
                          ...
                          icon: InkWell(
                              onTap: () {
                                setState(() {
                                  ...
                                  List<Supermarches> marche = supermarcheTrouve
                                      .where((element) =>
                                          element.article[0].nomArticle ==
          ...
          Expanded(
            child: Center(
              child: (!exec)
                  ? FutureBuilder<List<Supermarches>>(
                          ...
                          child: Card(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    supermarcheTrouve[index]
                                        .nom_part
                                        .toString(),
                                    style: TextStyle(fontSize: 20.0),
                                  ),
                                  Text(
                                    "Article : ${supermarcheTrouve[index].article[0].nomArticle.toString()} = ${supermarcheTrouve[index].article[0].prixArticle.toString()} Frcfa",
                                    style: TextStyle(
                                        color: Colors.lightBlueAccent.shade400),
                                  ),
                                  Text(
                                    " vous etes a  : ${supermarcheTrouve[index].infousersuper.toString()} kilometres",
                                    style:
  ...
  maRecherche() async {
    ...
    for (int i = 0; i <= supermarcheRepertorie.length; i++) {
      print(
          " latitude ${supermarcheRepertorie[i].latitude} ... longitude ${supermarcheRepertorie[i].longitude}");
      double distancesInMeters = Geolocator.distanceBetween(
          userLatitude,
          userLongitude,
          supermarcheRepertorie[i].latitude,
          supermarcheRepertorie[i].longitude);
...
//recuperer la longitude et la latitude de l'user afin de les convertir en la ville qui correspond
  ...
  List<Supermarches> searchArticle(List<Supermarches> supermarcheTrouve) {
    ...
    marche.forEach((element) {
      print("resultatttt ${element.nom_part}");
    });
    return marche;
  }
}
                    ...
                    child: Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              supermarches[index].nom_part.toString(),
                              style: TextStyle(fontSize: 20.0),
                            ),
                            Text(
                              supermarches[index].ville_part.toString(),
                              style: TextStyle(
                                  color: Colors.lightBlueAccent.shade400),
                            ),
                            Text(
                              " vous etes a  : ${supermarches[index].infousersuper.toString()} kilometres",
